...
let AuthenticationResolver = exports.AuthenticationResolver = class AuthenticationResolver {
    ...
    async createUser(updateAuthenticationInput, context) {
        if (context.req.userId) {
            let { avatar } = updateAuthenticationInput, restupdateAuthenticationInput = __rest(updateAuthenticationInput, ["avatar"]);
            avatar = avatar ?
                'http://localhost:4000/uploads/' + await (0, upload_utils_1.saveBase64ToFile)(avatar, context.req.userId)
                :
                    'http://localhost:4000/uploads/default_avatar.jpg';
            try {
                const updateUserDataInput = Object.assign(Object.assign({}, restupdateAuthenticationInput), { id: context.req.userId, connection_status: authentication_service_1.__ACCESS__, avatar, state: authentication_service_1.__CONNECTED__ });
